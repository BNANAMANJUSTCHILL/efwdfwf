...
public class TPACommand implements CommandExecutor {
    ...
    public TPACommand(TPAStorage storage, AvoidTPA plugin, ConfigManager configManager) {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args.length != 1) {
            p.sendMessage(configManager.getMessage("usage-tpa"));
            return true;
        }
        ...
        storage.addRequest(target.getUniqueId(), p.getUniqueId(), false);
        ...
        p.sendMessage(configManager.getMessage("tpa-sent", replacements));
        target.sendMessage(configManager.getMessage("tpa-received", replacements));
